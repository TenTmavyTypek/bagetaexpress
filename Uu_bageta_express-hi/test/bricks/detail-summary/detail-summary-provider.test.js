import Uu_bageta_express from "Uu_bageta_express-hi";
import { mount, wait } from "uu5g05-test";

describe(`Uu_bageta_express.Bricks.DetailSummary.DetailSummaryProvider`, () => {
  // it("onLoad", async () => {
  //   let onLoad = jest.fn(async () => ({ content: "Hello, world!" }));
  //   let loadDtoIn = { foo: "bar" };
  //   let wrapper = mount(<Uu_bageta_express.Bricks.DetailSummary.DetailSummaryProvider onLoad={onLoad} loadDtoIn={loadDtoIn} />);
  //   expect(onLoad).toHaveBeenCalledTimes(1);
  //   expect(onLoad).lastCalledWith(loadDtoIn);
  //   await wait();
  //   expect(wrapper.text()).toContain("Hello, world!");
  // });
});
